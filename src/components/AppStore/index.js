...
class AppStore extends Component {
  ...
  onChageApp = tabId => {
    this.setState({upadatTabId: tabId})
  }

  getFilterApp = () => {
    const {upadatTabId} = this.state

    const fiterAppsChage = appsList.filter(
      eachApp => eachApp.category === upadatTabId,
    )

    return fiterAppsChage
  }
  ...
  render() {
    const {upadatTabId, searchInput} = this.state
    const searchAppNames = this.getSearchedApps()

    const fiterAppsChage = this.getFilterApp(searchAppNames)
        ...
        <ul className="tab-item-container">
          {tabsList.map(eachTab => (
            <TabItem
              tabName={eachTab}
              key={eachTab.tabId}
              onChange={this.onChageApp}
              isActive={upadatTabId === eachTab.tabId}
            />
          ))}
        </ul>

        <ul className="apps-container">
          {fiterAppsChage.map(eachApp => (
